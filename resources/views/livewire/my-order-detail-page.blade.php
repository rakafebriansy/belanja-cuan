          ...
          <div class="grow">
            ...
            <div class="mt-1 flex items-center gap-x-2">
              <h3 class="text-xl font-medium text-gray-800 dark:text-gray-200">
                {{ $order_details[0]->created_at->format('d-m-Y') }}
              </h3>
            </div>
          </div>
        </div>
          ...
          <div class="grow">
            ...
            @php
                ...
                if($order->status == 'ordered') {
                    $status = '<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">Ordered</span>';
        ...
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <table class="w-full">
            ...
            <tbody>
              ...
              <tr wire:key="{{ $order_detail->id }}">
                <td class="py-4">
                  <div class="flex items-center">
                    <img class="h-16 w-16 mr-4" src="{{ url('storage', $order_detail->product->images[0]) }}" alt="{{ $order_detail->product->name }}">
                    <span class="font-semibold">{{ $order_detail->product->name }}</span>
                  </div>
                </td>
                <td class="py-4">{{ Number::currency($order_detail->unit_amount,'IDR') }}</td>
                <td class="py-4">
                  <span class="text-center w-8">{{ $order_detail->quantity }}</span>
                </td>
                <td class="py-4">{{ Number::currency($order_detail->total_amount,'IDR') }}</td>
              </tr>
              @endforeach
            </tbody>
          </table>
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>{{ Number::currency($order_detail->order->grand_total,'IDR') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxes</span>
          ...
          <div class="flex justify-between mb-2">
            ...
            <span class="font-semibold">{{ Number::currency($order_detail->order->grand_total,'IDR') }}</span>
          </div>
        </div>
      </div>
    </div>
